...
x-kit 플러그인별로 기록해야 할 이벤트:

### x-op 전략 실행
| 이벤트 | trace 타입 | 기록 시점 |
|--------|-----------|----------|
| 전략 시작 | `session_start` | `/x-op {strategy}` 호출 직후 |
| fan-out 호출 | `fan_out` | N개 에이전트 동시 호출 시 (agents[], prompt 포함) |
| delegate 호출 | `agent_call` | 단일 에이전트 호출 시 (role, model, prompt) |
| 라운드 종합 | `synthesize` | 리더가 결과 종합 시 (round, summary) |
| 체크포인트 저장 | `checkpoint` | `--resume` 용 체크포인트 저장 시 |
| 전략 완료 | `session_end` | 최종 출력 + Self-Score 완료 시 (total_cost, duration) |

### x-build 태스크 실행
| 이벤트 | trace 타입 | 기록 시점 |
|--------|-----------|----------|
| run 시작 | `session_start` | `x-build run` 호출 시 (step_id, task_count) |
| 에이전트 스폰 | `agent_call` | 각 태스크 에이전트 스폰 시 (task_id, role, model) |
| 태스크 완료 | `agent_call` + `status: done` | `tasks update --status completed` 콜백 시 |
| 태스크 실패 | `agent_call` + `status: failed` | 실패 + error_message 포함 |
| step 완료 | `checkpoint` | 현재 step의 모든 태스크 완료 시 |

### x-review 리뷰
| 이벤트 | trace 타입 | 기록 시점 |
|--------|-----------|----------|
| 리뷰 시작 | `session_start` | `/x-review diff` 호출 시 |
| 관점별 리뷰 | `fan_out` | 에이전트별 리뷰 호출 시 (lens, model) |
| 종합 판정 | `synthesize` | verdict 결정 시 (findings_count, verdict) |

## 비용 알림 규칙

trace 기록 중 누적 비용이 임계값을 초과하면 경고:

| 임계값 | 동작 |
|--------|------|
| `budget × 50%` | `💡 비용 50% 사용: $X / $Y` (정보) |
| `budget × 80%` | `⚠ 비용 80% 사용: $X / $Y` (경고) |
| `budget × 100%` | `🚨 예산 초과: $X / $Y — 계속 진행할까요?` (확인 요청) |

budget은 `.xm/config.json`의 `budget.max_usd`에서 읽는다. 미설정 시 알림 없음.
