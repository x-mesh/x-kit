...
| [x-agent](#x-agent) | 에이전트 프리미티브 & 팀 | `/x-agent fan-out "작업"` |
| [x-trace](#x-trace) | 실행 추적 & 비용 | `/x-trace timeline` |
| [x-memory](#x-memory) | 크로스 세션 메모리 | `/x-memory inject` |
...
프로젝트 전체 라이프사이클 — PRD 생성, 다중 모드 토론, 합의 리뷰, 수락 계약, 품질 게이트 실행.
...
[상태+baseline] [반증 가능] [변수 하나] [전환/복원] [실행 검증] [왜 늦었나?]
```
...
이걸 만들어야 할까? 커밋하기 전에 검증하세요. 근거 등급 기반 질문, 도메인 인식 검증, 사전부검 분석과 구조화된 다운스트림 통합.
...
| 기능 | 설명 |
|---------|-------------|
| **6가지 사고 원칙** | 기본은 NO, 싸게 죽이기, 출처와 날짜가 있는 근거, 사전부검, 코드는 비싸다, 답하지 말고 물어라 |
...
[책임 확인]    [요약]    [실패 식별]   [근본 원인]   [강화된 대안]   [KEEP/STOP/START]
```
...
| **크로스 세션 패턴** | 반복되는 편향 태그 자동 표면화 |
| **Steelman 프로토콜** | 사용자가 먼저 대안 제시, 에이전트가 강화 |
| **Comfortable Challenger** | 에이전트가 자기 합리화에 직접 도전 |
| **KEEP/STOP/START** | 레슨 저장, 선택적으로 CLAUDE.md에 적용 |
...
기반 레이어. Claude Code 네이티브 Agent 도구 위의 구조화된 패턴 + 계층적 팀 관리.
...
x-build plan → PRD 품질 게이트 (7.0+) → 합의 리뷰 (4명 에이전트)
     ↓
x-build tasks done-criteria → PRD에서 수락 계약
     ↓
...
| 컴포넌트 | 메커니즘 |
|-----------|-----------|
| **Self-Score** | 모든 x-op 전략이 매핑된 루브릭 대비 자동 채점 |
| **--verify 루프** | 심사 패널 (편향 인식) → 실패 → 피드백 → 재실행 (최대 2회) |
| **PRD 합의** | architect + critic + planner + security, 원칙 기반 프롬프트 |
| **수락 계약** | `done_criteria`를 PRD에서 자동 도출 → 에이전트에 주입 → 종료 시 검증 |
| **자동 핸드오프** | 페이즈 전환 시 결정은 보존, 탐색 노이즈는 버림 |
| **plan-check (11차원)** | 원자성, 의존성, 커버리지, 세분도, 완전성, 컨텍스트, 네이밍, 기술 누출, 스코프 명확성, 리스크 순서, 종합 |
| **품질 대시보드** | `x-build status`로 태스크별 점수 + 프로젝트 평균 |
| **도메인 루브릭** | 5가지 프리셋 (api-design, frontend, data-pipeline, security, architecture) |
| **편향 인식 심사** | x-humble 레슨 (확인 3회+)이 심사 컨텍스트에 반영 |
...
```
x-kit/                              마켓플레이스 레포
├── x-build/                        프로젝트 하니스 + PRD 파이프라인
├── x-op/                           전략 오케스트레이션 (18가지 전략)
├── x-eval/                         품질 평가 + diff
├── x-humble/                       구조화된 회고
├── x-solver/                       문제 해결 (4가지 전략)
├── x-agent/                        에이전트 프리미티브 & 팀
...
- **Claude**: SKILL.md를 해석하고, Agent Tool로 에이전트를 스폰하며, 완료 시 CLI 콜백 호출.
...
```bash
/x-build steps compute            # 태스크 의존성에서 DAG 빌드
```
