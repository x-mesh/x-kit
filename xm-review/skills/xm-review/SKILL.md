...
<Purpose>
xm-review는 PR diff, 파일, 또는 디렉토리를 입력받아 다수의 리뷰 에이전트를 병렬로 구동한다. 각 에이전트는 전담 관점(보안, 로직, 성능, 테스트 커버리지)에서 발견사항을 severity + file:line 형식으로 보고하고, 리더가 통합 보고서와 LGTM / Request Changes / Block 판정을 발행한다.
</Purpose>
...
- Other xm-kit skills need a code quality gate
</Use_When>

<Do_Not_Use_When>
- Simple single-line questions that don't need multi-agent review
- Structured problem solving (use xm-solver instead)
- Full project lifecycle management (use xm-build instead)
</Do_Not_Use_When>

# xm-review — Multi-Perspective Code Review
...
```
xm-review — Multi-Perspective Code Review Orchestrator
...
Examples:
  /xm-review diff
  /xm-review diff HEAD~3
  /xm-review pr 142
  /xm-review file src/auth.ts
  /xm-review diff --lenses "security,logic" --severity high
  /xm-review pr 142 --format github-comment --agents 2
```
...
```
Agent tool 1: {
  description: "xm-review: security",
  prompt: "## Code Review: Security\n\n{diff_content}\n\n[관점 프롬프트]",
  run_in_background: true,
  model: "sonnet"
}
Agent tool 2: {
  description: "xm-review: logic",
...
```
🔍 [xm-review] Complete — {N} agents, {M} findings
...
*Generated by [xm-review](https://github.com/x-mesh/xm-kit)*
</details>
````
...
xm-review는 `.xm/config.json`의 공유 설정을 참조한다:

| 설정 | 키 | 기본값 | 영향 |
|------|-----|--------|------|
| 에이전트 수 | `agent_level` | `medium` (4) | `--agents` 미지정 시 기본 에이전트 수 결정 |
...
xm-build Verify 페이즈에서 품질 게이트로 활용:
